...
class BillingAddressController(DataController):
    def __init__(self):
        sys.setrecursionlimit(10**6)
        super().__init__()
        self.utils = UtilsController()
        self.log = LogsController()
        self.module_name = "OrderBillingAddressController"
    ...
    # Database Functions
    def get_all_billing_addresses(self):
        ...
        try:
            result_flag, result_query = super().query_record(super().get_tbl_BILLING_ADDRESS(), columns, condition)
    ...
    def get_specific_billing_address(self, order_id):
    ...
    def insert_billing_address(self, order_id, address: BillingAddress):
        # print(f"\n[INFO] BEGIN - Inserting Billing Address")
        result_flag = False
        ...
        try:
            ...
            result_flag, rowcount, result_string = super().insert_record(super().get_tbl_BILLING_ADDRESS(), self.utils.get_columns_array(), self.utils.get_values_array())

            if result_flag:
                print(f"[INFO] Billing Address Inserted...\t\tOrder: {order_id}")
            else:
                if "1062" in result_string or "Duplicate entry" in result_string:
                    if "PRIMARY" in result_string:
                        while keep_trying:
                            count_try = count_try + 1
                            print(f"[INFO] Trying to insert Billing Address again...\tOrder: {order_id}.Try: {count_try}")
                            ...
                            if result_flag == True:
                                ...
                                if "ID" in result_string:
                                    print(f"[INFO] Updating Billing Address...\t\tOrder: {order_id}")
                                    self.utils.clear_condition()
                                    self.utils.set_condition(f"ORDER_ID = '{order_id}'")
                                    result_flag, rowcount, result_string = super().update_record(super().get_tbl_BILLING_ADDRESS(), self.utils.get_columns_array(), self.utils.get_values_array(), self.utils.get_condition())

                                    if result_flag == True:
                                        print(f"[INFO] Billing Address Updated...\t\t\tOrder: {order_id}")
                                        keep_trying = False
                                    else:
                                        if count_try >= maximum_insert_try:
                                            print(f"[INFO] Billing Address NOT Inserted/Maximum tries...\tOrder: {order_id}.Try: {count_try}")
                                            keep_trying = False
                                else:
                                    if count_try >= maximum_insert_try:
                                        print(f"[INFO] Billing Address NOT Inserted/Maximum tries...\tOrder: {order_id}.Try: {count_try}")
                    ...
                    print(f"[ERROR] Error inserting Billing Address...\tOrder: {order_id}")
            ...
            # print(f"[INFO] END - Finished inserting Billing Address")
            # print("[INFO] Cleaning up variables")
            try:
    ...
    def update_billing_address(self, order_id, address: BillingAddress):
        # print(f"\n[INFO] BEGIN - Updating Billing Address")
        result_flag = False
        result_string = "Success"
        condition = f"ORDER_ID = '{order_id}'"
        rowcount = 0
        ...
        try:
            ...
            result_flag, rowcount, result_string = super().update_record(super().get_tbl_BILLING_ADDRESS(), self.utils.get_columns_array(), self.utils.get_values_array(), condition)

            if result_flag:
                print(f"[INFO] Billing Address Updated...\t\tOrder: {order_id}")
            else:
                print(f"[ERROR] Error updating Billing Address...\tOrder: {order_id}")
            ...
            # print(f"[INFO] END - Finished updating Billing Address")
            # print("[INFO] Cleaning up variables")
            try:
    ...
    def delete_billing_address(self, order_id):
        # print(f"\n[INFO] BEGIN - Deleting Billing Address")
        result_flag = False
        result_string = "Success"
        condition = f"ORDER_ID = '{order_id}'"
        rowcount = 0
        ...
        try:
            result_flag, rowcount, result_string = super().delete_record(super().get_tbl_BILLING_ADDRESS(), condition)
            if result_flag:
                print(f"[INFO] Billing Address Deleted...\t\tOrder: {order_id}")
            else:
                print(f"[ERROR] Error deleting Billing Address...\tOrder: {order_id}")
            ...
            # print(f"[INFO] END - Finished deleting Billing Address")
            # print("[INFO] Cleaning up variables")
            try:
    ...
    def verify_billing_address_exists(self, order_id):
        # print(f"\n[INFO] BEGIN - Verifying Billing Address exists")
        ...
        try:
            ...
            # print(f"[INFO] END - Finished verifying Billing Address exists")
            # print("[INFO] Cleaning up variables")
            try:
    ...
    def upsert_billing_address(self, order_id, address: BillingAddress):
        # print(f"\n[INFO] BEGIN - Upserting Billing Address")
        result_flag = False
        rowcount = 0
        result_string = "Success"
        ...
        try:
            address_exists = self.verify_billing_address_exists(order_id)
            if address_exists:
                result_flag, rowcount, result_string = self.update_billing_address(order_id, address)
            else:
                result_flag, rowcount, result_string = self.insert_billing_address(order_id, address)
            ...
            # print(f"[INFO] END - Finished upserting Billing Address")
            # print("[INFO] Cleaning up variables")
            try:
    ...
    # API Functions
    def upsert_billing_address_api(self, order_id, address_json):
        # print(f"\n[INFO] BEGIN - Upserting Billing Address")
        ...
        try:
            ...
            result_flag, rowcount, result_string = self.upsert_billing_address(order_id, address)
            if result_flag:
    ...
    def add_billing_address(self, order_id, address_json):
        # print(f"\n[INFO] BEGIN - Adding Billing Address")
        address = self.BillingAddress()
        result_flag = False
        rowcount = 0
        result_string = "Success"
        ...
        try:
            address_exists = self.verify_billing_address_exists(order_id)
            if address_exists:
                return False, 409, 0, "Billing Address already exists"
            else:
                # address.order_id = order_id
                address.first_name = first_name
                address.last_name = last_name
            ...
            # print(f"[INFO] END - Finished adding Billing Address")
            # print("[INFO] Cleaning up variables")
            try:
                del address, address_exists, result_flag, rowcount, result_string, order_id, first_name, last_name, name, company, address1, address2, city, zip, province, province_code, country, country_code, phone, latitude, longitude
            except:
    ...
    def edit_billing_address(self, order_id, address_json):
        # print(f"\n[INFO] BEGIN - Editing Billing Address")
        address = self.BillingAddress()
        result_flag = False
        rowcount = 0
        result_string = "Success"
        ...
        try:
            ...
            if not address_exists:
                return False, 404, 0, "Billing Address not found!"
            else:
                # address.order_id = order_id
                address.first_name = first_name
                address.last_name = last_name
            ...
            # print(f"[INFO] END - Finished editing Billing Address")
            # print("[INFO] Cleaning up variables")
            try:
    ...
    def remove_billing_address(self, order_id):
        # print(f"\n[INFO] BEGIN - Removing Billing Address")
        result_flag = False
        rowcount = 0
        result_string = "Success"
        address_exists = False
        ...
        try:
            ...
            if not address_exists:
                ...
                result_flag, rowcount, result_string = self.delete_billing_address(order_id)
                if result_flag:
                    return result_flag, 200, rowcount, result_string
                else:
                    return result_flag, 500, rowcount, result_string
            ...
            # print(f"[INFO] END - Finished removing Billing Address")
            # print("[INFO] Cleaning up variables")
            try:
    ...
    def get_all_billing_addresses_api(self):
        # print(f"\n[INFO] BEGIN - Getting all billing addresses")
        result_flag = False
        result = None

        try:
            result_flag, result = self.get_all_billing_addresses()
            if result_flag:
    ...
    def get_specific_billing_address_api(self, order_id):
        # print(f"\n[INFO] BEGIN - Getting specific billing address")
        result_flag = False
        result = None
        address_exists = False
        ...
        try:
            address_exists = self.verify_billing_address_exists(order_id)
            if not address_exists:
                return False, 404, "Billing Address not found!"
            else:
                result_flag, result = self.get_specific_billing_address(order_id)
                if result_flag:
                    return result_flag, 200, result
                else:
                    return result_flag, 500, result
